...
- Steps are called inside `hotest {}`.
- The test uses a classic `@Test`.

Note that any xUnit-style framework can run HOTest scenarios: JUnit, Kotest, Kotlin test, etc.
...
- Traversal order follows depth-first search (DFS) through nested variants.
...
1. You can define any number of `variant {}` blocks inside a `variants {}`.
1. Only one `variants {}` block is allowed at a given test level.
1. `variants {}` can be nested inside other `variants {}` blocks - for example:
...
**Real usage example**
- Instruction how to set up your project to use HOTest and real world scenarios you can find in associated project [Multi Project Focus](https://github.com/gt4dev/MultiProjectFocus)
